...
def perform_calculation(state):
    ...
    state.multiplication = int(state.number1) * int(state.number2)
    state.division = int(state.number1) / int(state.number2)
    state.subtraction = int(state.number1) - int(state.number2)
    ...
    with tgb.part(class_name="container card"):
        with tgb.layout(columns="1 1 1"):
            ...
            with tgb.part() as column_calculator:
                ...
                tgb.text("{number1} * {number2} = {multiplication}")
                tgb.text("{number1} / {number2} = {division}")
                tgb.text("{number1} - {number2} = {subtraction}")
